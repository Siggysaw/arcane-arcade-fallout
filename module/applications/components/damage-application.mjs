...
export default class DamageApplicationElement extends ChatTrayElement {
  ...
  buildTargetListEntry(uuid) {
    ...
    for (const [change, values] of Object.entries(active)) {
      for (const type of values) {
        ...
        if (!config) continue
        const data = { type, change, icon: config.img }
        types.push(data)
      }
    }
    const changeSources = types.reduce((acc, { type, change, icon }) => {
      ...
      acc += `
        <button class="change-source unbutton" type="button" data-type="${type}" data-change="${change}"
          ...
          <aafo-icon src="${icon}" inert></aafo-icon>
    ...
    li.innerHTML = `
      ...
      <div class="calculated temp" data-tooltip="AAFO.HitPointsTemp">
        ${temp}
      </div>
  ...
  getChangeSourceOptions(type, change, options) {
    ...
    let label = game.i18n.format(`AAFO.DamageApplication.Change.${change.capitalize()}`, {
      type: CONFIG.AAFO.damageTypes[type]?.label ?? CONFIG.AAFO.healingTypes[type]?.label,
    })
    if (mode === 'ignore')
      label = game.i18n.format('AAFO.DamageApplication.Ignoring', { source: label })
    if (mode === 'downgrade')
      label = game.i18n.format('AAFO.DamageApplication.Downgrading', { source: label })
  ...
  refreshListEntry(token, entry, options) {
    ...
    calculatedDamage.dataset.tooltip = `AAFO.${total < 0 ? 'Healing' : 'Damage'}`
