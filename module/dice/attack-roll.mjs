...
export default class AttackRoll extends FormApplication {
  constructor(actor, weapon, options = {}, callback = () => { }) {
    ...
    this.formDataCache = {
      ...
      totalApCost: this.getBaseApCost(),
      adjustedApCost: 0,
      ammoCost: 1,
      totalAmmoCost: 1,
      adjustedAmmoCost: 0,
        ...
        render: (html) => {
          ...
          buttons.forEach((button) => {
            button.addEventListener('click', (e) => {
              ...
              this.formDataCache.totalApCost = this.getBaseApCost() + targetedAp
              this.render()
              dlg.close()
            })
          })
  ...
  activateListeners($html) {
    ...
    removeTarget?.addEventListener('click', () => {
      ...
      this.formDataCache.totalApCost = this.getBaseApCost()
      this.render()
    })
 ...
 //Returns true if `text` appears in either the weapon's description or its bonusProperties field.
  ...
  getFlavor(target, hit, diceRoll) {
    ...
    if (this.weapon.type === 'explosive') {
      const explosive = this.weapon.name 
      diceRoll < 2 ? flavor = `KABOOM! ${explosive} EXPLODED IN YOUR HAND!` :
        diceRoll == 2 ? flavor = `KABOOM! ${explosive} detonated halfway to the target!` :
          diceRoll > 2 && diceRoll < 15 ? flavor = `KABOOM! ${explosive} will detonate at the start of your next turn!` :
            flavor = `KABOOM! ${explosive} detonates at the end of your turn!`
    return flavor
    } else {
  ...
  async performRoll() {
    ...
    const hitSummary = hitResults.length
      ? `<div class="hit-results">${hitResults
          .map((r) => `<div>${r.hit ? 'Hits' : 'Misses'} ${r.token.name}</div>`)
          .join('')}</div>`
      : ''
  ...
  `
    ...
    const attackMessage = await roll.toMessage({
      speaker: ChatMessage.getSpeaker({ actor: this.actor }),
      flavor: (this.getFlavor(this.formDataCache.targeted?.target, anyHit, roll.total) || '') + hitSummary,
      rollMode: game.settings.get('core', 'rollMode'),
      'flags.falloutzero': {
        type: 'attack',
        itemId: this.weapon.id,
